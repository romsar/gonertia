	...
	t.Run("plain request", func(t *testing.T) {
		...
		t.Run("ssr", func(t *testing.T) {
			...
			newTestServerSSR := func(t *testing.T) *httptest.Server {
				...
				return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
					...
					ssr := map[string]any{
						...
						"body": `<script data-page="app" type="application/json">` + string(pageJSON) + `</script><div id="app">foo bar</div>`,
					}
			...
			successRunner := func(t *testing.T, i *Inertia) {
				...
				re := regexp.MustCompile(`<div\sid="app">([^<]+)</div>`)
	...
	t.Run("inertia request", func(t *testing.T) {
		...
		t.Run("props value resolving", func(t *testing.T) {
			...
			t.Run("once props", func(t *testing.T) {
				...
				t.Run("included on first page load", func(t *testing.T) {
					t.Parallel()
				...
				t.Run("excluded on partial reloads", func(t *testing.T) {
					t.Parallel()
				...
				t.Run("included when requested (via only)", func(t *testing.T) {
					t.Parallel()

					w, r := requestMock(http.MethodGet, "/home")
					asInertiaRequest(r)
					withPartialComponent(r, "Some/Component")
					withOnly(r, []string{"foo"})
